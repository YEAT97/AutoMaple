...
class Bot(Configurable):
    ...
    def _main(self):
        ...
        while True:
            if config.enabled and len(config.routine) > 0:
                ...
                #if config.low_hp:  
                #    press(self.config['HP'], 2, down_time=0.2)  
                #    config.low_hp = False  
                    
                #if config.low_mp:  
                #    press(self.config['MP'], 2, down_time=0.2)  
                #    config.low_mp = False  
    ...
    def _solve_rune(self, model):
        ...
        osk_window = gw.getWindowsWithTitle('On-Screen Keyboard')[0]
        osk_left = osk_window.left  
        osk_top = osk_window.top               
        ...
        print('\nSolving rune:')
        time.sleep(0.5)  #  等待符文完全显示
        frame = config.capture.frame
        solution = detection.merge_detection(model, frame, debug=True)
        ...
        if solution:
            ...
            for arrow in solution:  
                try:
                    x, y = key_coordinates[arrow]
                    key_x = osk_left + x
                    key_y = osk_top + y
                    click((key_x, key_y))
                    time.sleep(0.2)  
                except KeyError:
                    print(f"Error: Invalid arrow direction '{arrow}'")
            time.sleep(1)  
        else:
            print("Error: Could not detect rune solution.") 
